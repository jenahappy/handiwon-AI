...
window.AI = (() => {
  ...
  // Vercel /api/image 프록시 URL (HF 무료 이미지 → 실패 시 Pollinations 자동 대체)
  function imageProxyUrl(prompt, opts = {}) {
    const full = decoratePrompt(prompt);
    const w = opts.width || 768, h = opts.height || 768;
    const seed = opts.seed != null ? opts.seed : Math.floor(Math.random() * 1e6);
    const base = C.PROXY_URL.replace(/\/$/, "") + "/image";
    const p = new URLSearchParams({ prompt: full, width: String(w), height: String(h), seed: String(seed) });
    return base + "?" + p.toString();
  }

  // 이미지 소스 결정(Promise<string>)
  //  - 프록시 설정 시: /api/image (HF 무료 → 실패 시 Pollinations)
  //  - 프록시 없을 때: Pollinations 직접
  async function resolveImageSrc(prompt, opts = {}) {
    if (C.PROXY_URL) return imageProxyUrl(prompt, opts);
    return imageUrl(prompt, opts);
  }

  function isConfigured() { return !!(C.PROXY_URL || C.DEV_GROQ_KEY); }

  return { chat, chatJSON, imageUrl, resolveImageSrc, isConfigured };
})();
